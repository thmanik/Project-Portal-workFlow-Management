...
import { CheckCircle2, Download, Eye, FileText, Filter, Mail, MoreHorizontal, Paperclip, Plus, Search, Upload, XCircle } from "lucide-react";
...
type TypeFilter = "ALL" | "BID" | "PROJECT" | "PENDING" | "ACTIVE_CLIENT" | "REVISION" | "ARCHIVE";
...
function allWorkRequestsAccepted(relatedRequests: WorkRequest[]) {
  return relatedRequests.length > 0 && relatedRequests.every((request) => request.currentStatus === "HML_LISTED");
}

function hasClientRevisionRequest(relatedRequests: WorkRequest[]) {
  return relatedRequests.some((request) => request.currentStatus === "CLIENT_REVISION_REQUESTED");
}
...
function ProjectsPage() {
  ...
  const { state, currentActor, addProject, addClientDocument, decideBidOutcome, requestArchivedBidReview } = usePortal();
  ...
  const isClientMode = currentActor.role === "client_owner";
  ...
  const visibleProjects = useMemo(() => {
    ...
    if (isClientMode) {
      list = list.filter((project) => {
        const relatedRequests = state.workRequests.filter((request) => request.parentId === project.id);
        const hasRevision = hasClientRevisionRequest(relatedRequests);

        if (typeFilter === "PENDING") return project.status === "DRAFT" && hasFinalHmlListing(relatedRequests);
        if (typeFilter === "ACTIVE_CLIENT") return project.status !== "ARCHIVED" && project.status !== "DRAFT" && !hasRevision;
        if (typeFilter === "REVISION") return hasRevision && project.status !== "ARCHIVED";
        if (typeFilter === "ARCHIVE") return project.status === "ARCHIVED";
        return true;
      });
    } else {
  ...
  }, [currentActor.clientId, currentActor.role, isClientMode, search, state.clients, state.projects, state.workRequests, statusFilter, typeFilter]);
  ...
  const handleProjectFileChange = async (idx: number, files?: FileList | null) => {
    ...
    try {
      ...
      setProjectForm((prev) => {
        const baseDoc = prev.docs[idx] || createEmptyDocumentRow(state.settings.categories[0] || "General");
        const uploadedDocs = fileRows.map(({ file, fileDataUrl }, fileIndex) => ({
          ...baseDoc,
          name: fileIndex === 0 ? baseDoc.name || file.name : file.name,
          textContent: fileIndex === 0 ? baseDoc.textContent : "",
          fileName: file.name,
        ...
        docs.splice(idx, 1, ...uploadedDocs);
        return { ...prev, docs };
      });
    } catch {
        ...
        <div className="inline-flex w-full rounded-lg border border-input bg-background p-1 lg:w-auto">
          {(isClientMode
            ? [
                ...
                { label: "Active", value: "ACTIVE_CLIENT" as const },
          ...
          <table className="w-full min-w-[980px] text-left text-sm">
            ...
            <tbody className="divide-y divide-border">
              {visibleProjects.map((project) => {
                ...
                const isClientPendingAcceptedBid = isClientMode && project.type === "BID" && project.status === "DRAFT" && hasFinalHmlListing(relatedWorkRequests);
                const canActivateClientBid = allWorkRequestsAccepted(relatedWorkRequests);
                const canDecideBidOutcome = canManage && !isClientMode && project.type === "BID" && project.status !== "ARCHIVED" && project.status !== "DRAFT" && hasFinalHmlListing(relatedWorkRequests);
                const canRequestArchiveReview = canManage && !isClientMode && project.status === "ARCHIVED";
                  ...
                  <tr key={project.id} className="bg-card transition-colors hover:bg-muted/30">
                    ...
                    <td className="px-5 py-4 align-top text-right">
                      <div className="flex flex-wrap items-center justify-end gap-2">
                        {isClientPendingAcceptedBid ? (
                          <>
                            <Button
                              ...
                              disabled={!canActivateClientBid}
                              title={canActivateClientBid ? undefined : "All work requests must be accepted before assignment"}
                              onClick={() => {
                                if (!canActivateClientBid) return;
                                decideBidOutcome(project.id, "WIN");
                                toast.success("Bid assigned", {
                                  description: `${project.code} moved to Active Projects.`,
                                });
                              }}
                            >
                            ...
                            <Button
                              ...
                              disabled={!canActivateClientBid}
                              title={canActivateClientBid ? undefined : "All work requests must be accepted before project rejection"}
                              onClick={() => {
                                if (!canActivateClientBid) return;
                        ...
                        {canRequestArchiveReview ? (
                          <Button
                            variant="outline"
                            size="sm"
                            className="h-8 text-xs"
                            onClick={() => {
                              requestArchivedBidReview(project.id);
                              toast.success("Review requested", {
                                description: `${project.code} moved back to Bid review for client discussion.`,
                              });
                            }}
                          >
                            Review Request
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-40">
                          <DropdownMenuItem onSelect={() => navigate({ to: "/documents", search: { projectId: project.id } })}>
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                      ...
                      <div>
                        ...
                        <input
                          type="file"
                          multiple
                          onChange={(event) => void handleProjectFileChange(idx, event.target.files)}
                        ...
                        <p className="mt-1 text-xs text-muted-foreground">You can select multiple files at once. Each file will be saved as a separate primary document.</p>
                      </div>
