...
function Dashboard() {
  ...
  const activeWorkflowSteps = latestRequest
    ? [
        { label: "Marketing Intake", department: "CCR", status: "completed" as const, timestamp: formatDate(latestRequest.revisionHistory[latestRequest.revisionHistory.length - 1]?.at || latestRequest.lastTransferredAt) },
        { label: "PM Lead Review", department: "ECM / PMO", status: ["DIVISION_NOTIFIED", "LEADER_ASSIGNED", "MARKETING_INFO_PROVIDED"].includes(latestRequest.currentStatus) ? "active" as const : (latestRequest.currentStatus === "CREATED" ? "pending" as const : "completed" as const) },
        { label: "Info Loop", department: "Client / CCR / PM", status: ["INFO_REQUESTED_FROM_MARKETING", "INFO_REQUESTED_FROM_CLIENT", "CLIENT_INFO_PROVIDED"].includes(latestRequest.currentStatus) ? "active" as const : (["MARKETING_INFO_PROVIDED", "MEMBER_REVIEW", "PM_MEMBER_SUBMITTED", "FORWARDED_TO_TMS", "TMS_ASSIGNED", "DRAWING_IN_PROGRESS", "CHECKING_REVIEW", "APPROVAL_REVIEW", "ENGINEERING_LEAD_REVIEW", "FINAL_SUBMITTED_TO_MARKETING", "FINAL_SUBMITTED_TO_CLIENT", "HML_LISTED"].includes(latestRequest.currentStatus) ? "completed" as const : "pending" as const) },
        { label: "PM Member + QC", department: "ECM / PMO", status: ["MEMBER_REVIEW", "MEMBER_INFO_REQUESTED", "PM_LEAD_RESPONDED", "PM_MEMBER_SUBMITTED", "PM_REWORK_REQUESTED"].includes(latestRequest.currentStatus) ? "active" as const : (["FORWARDED_TO_TMS", "TMS_ASSIGNED", "DRAWING_IN_PROGRESS", "CHECKING_REVIEW", "APPROVAL_REVIEW", "ENGINEERING_LEAD_REVIEW", "FINAL_SUBMITTED_TO_MARKETING", "FINAL_SUBMITTED_TO_CLIENT", "HML_LISTED"].includes(latestRequest.currentStatus) ? "completed" as const : "pending" as const) },
        { label: "Engineering", department: "TMS", status: ["FORWARDED_TO_TMS", "ENGINEERING_REVISION_REQUESTED", "TMS_ASSIGNED", "DRAWING_IN_PROGRESS", "CHECKING_REVIEW", "APPROVAL_REVIEW", "ENGINEERING_LEAD_REVIEW"].includes(latestRequest.currentStatus) ? "active" as const : (["FINAL_SUBMITTED_TO_MARKETING", "FINAL_SUBMITTED_TO_CLIENT", "HML_LISTED"].includes(latestRequest.currentStatus) ? "completed" as const : "pending" as const) },
        { label: "Final Delivery", department: "CCR / Client", status: ["FINAL_SUBMITTED_TO_MARKETING", "FINAL_SUBMITTED_TO_CLIENT", "CLIENT_REVISION_REQUESTED"].includes(latestRequest.currentStatus) ? "active" as const : (latestRequest.currentStatus === "HML_LISTED" ? "completed" as const : "pending" as const) },
        { label: "Complete", department: "HML", status: latestRequest.currentStatus === "HML_LISTED" ? "completed" as const : "pending" as const },
      ]
    : [];
  ...
  return (
    <div>
      <PageHeader
        title="Dashboard"
        description="Operational overview for Client, CCR / Marketing, ECM / PMO, TMS Engineering, and HML closeout."
        actions={
          canManage ? (
            <Link to="/projects">
              <Button>
